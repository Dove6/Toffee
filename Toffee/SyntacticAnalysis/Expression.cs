...
public record ConditionalExpression(ConditionalElement IfPart, IList<ConditionalElement> ElifParts,
    Expression? ElsePart = null) : Expression(new Position());
public record ForLoopExpression(ForLoopRange Range, Expression Body, string? CounterName = null)
    : Expression(new Position());
public record WhileLoopExpression(Expression Condition, Expression Body) : Expression(new Position());
...
public record PatternMatchingExpression(Expression Argument, IList<PatternMatchingBranch> Branches,
    Expression? Default = null) : Expression(new Position());
